...
const Position TirionPos = {746.541f, 556.526f, 436.001f, 1.56f};
const Position GarroshPos = {686.053f, 615.201f, 436.001f, 0.41f};
const Position ThrallPos = {685.619f, 622.174f, 436.001f, 5.93f};
const Position VarianPos = {807.670f, 623.194f, 436.001f, 3.41f};
const Position JainaPos  = {807.728f, 614.487f, 436.001f, 2.98f};
            ...
            if (GetData(EVENT_INTRO) == IN_PROGRESS)
            {
                switch(events.ExecuteEvent())
                {
                    case 1:
						...
						me->SummonCreature(NPC_TIRION, TirionPos);
						me->SummonCreature(NPC_THRALL, ThrallPos);
						me->SummonCreature(NPC_GARROSH, GarroshPos);
						me->SummonCreature(NPC_VARIAN, VarianPos);
						me->SummonCreature(NPC_JAINA, JainaPos);
                        break;
                    case 2:
                        if (!playersGUID.empty())
                        {
            ...
            if (GetData(EVENT_BLACK_KNIGHT_INTRO) == IN_PROGRESS)
            {
                switch(events.ExecuteEvent())
                {
                    case 1:
						...
						me->SummonCreature(NPC_BLACK_KNIGHT, SpawnPosition);
                        events.ScheduleEvent(2, 3000);
                        break;
                    case 2:
                    ...
                    case 4:
                        if (Creature* blackKinght = me->GetCreature(*me, instance->GetData64(DATA_BLACK_KNIGHT)))
                        {
							...
							blackKinght->SetTarget(me->GetGUID());
							blackKinght->AI()->DoCast(me->GetCreature(*me, instance->GetData64(DATA_ANNOUNCER)), 67745);
                        }
                        events.ScheduleEvent(5, 4000);
                        break;
                    case 5:
                        if (Creature* blackKinght = me->GetCreature(*me, instance->GetData64(DATA_BLACK_KNIGHT)))
                        {
                            // blackKinght->AI()->DoCast(me->GetCreature(*me, instance->GetData64(DATA_ANNOUNCER)), SPELL_DEATH_PUSH_INTRO);
                    ...
                    case 9:
                        ...
                        if (Creature* blackKinght = me->GetCreature(*me, instance->GetData64(DATA_BLACK_KNIGHT)))
                        {
                            ...
                            blackKinght->Kill(me);
							blackKinght->setFaction(16);
							AggroAllPlayers(blackKinght);
                        }
                        break;
                }
            }
